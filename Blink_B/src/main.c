...
void InitClock() 
{
	PM->CLK_APB_P_SET |= PM_CLOCK_UART_0_M | PM_CLOCK_GPIO_0_M | PM_CLOCK_GPIO_1_M | PM_CLOCK_GPIO_2_M; // включение тактирования GPIO
	PM->CLK_APB_M_SET |= PM_CLOCK_PAD_CONFIG_M | PM_CLOCK_WU_M | PM_CLOCK_PM_M | PM_CLOCK_WU_M; // включение тактирования блока для смены режима выводов
}
...
void ledBlink() 
{
	...
	#ifdef BOARD_DIP
	GPIO_1->OUTPUT |= 1 << PIN_LED;   // Установка сигнала вывода 7 порта 2 в высокий уровень
	for (volatile int i = 0; i < 100000; i++);
	GPIO_1->OUTPUT &= ~(1 << PIN_LED); // Установка сигнала вывода 7 порта  в низкий уровень   
	for (volatile int i = 0; i < 100000; i++);
	#endif 
}
...
int main() 
{
	...
	#ifdef BOARD_LITE_V0
	PAD_CONFIG->PORT_2_CFG = 1 << (2 * PIN_LED);
	...
	#ifdef BOARD_DIP
	PAD_CONFIG->PORT_1_CFG = 1 << (2 * PIN_LED);
	GPIO_1->DIRECTION_OUT = 1 << PIN_LED; 				// Установка направления вывода 7 порта 2 на выход
	GPIO_1->OUTPUT = 0;
	#endif 
