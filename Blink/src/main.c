...
void InitClock() 
{
	...
	PM->CLK_APB_M_SET |= PM_CLOCK_APB_M_PAD_CONFIG_M | PM_CLOCK_APB_M_WU_M | PM_CLOCK_APB_M_PM_M | PM_CLOCK_APB_M_WU_M; // включение тактирования блока для смены режима выводов
}
...
int main() 
{
	...
	#ifdef BOARD_LITE_V0
		// PAD_CONFIG->PORT_2_CFG |= (0 << (2 * PIN_LED)); 	// Установка вывода 7 порта 2 в режим GPIO
		GPIO_2->DIRECTION_OUT = 1 << PIN_LED; 				// Установка направления вывода 7 порта 2 на выход
		// PAD_CONFIG->PORT_2_CFG |= (0 << (2 * PIN_BUTTON)); 	// Установка вывода 6 порта 2 в режим GPIO
