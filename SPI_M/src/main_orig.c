...
int main()
{
    ...
    SPI_TypeDef* master = SPI_0;

    uint8_t maser_input[] = {0};
    ...
    SPI_Init(master, SPI_CONFIG_MASTER_M | SPI_CONFIG_BAUD_RATE_DIV_16_M | SPI_CONFIG_CS_0_M);

    while (1)
    {

        SPI_Exchange(master, master_data, maser_input, sizeof(master_data));
