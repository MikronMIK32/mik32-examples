...
void trap_handler() 
{
    ...
    xprintf("\ntrap_handler()\n");
    xprintf("Перывание вызвано %d раз\n", trap_handler_called);
    uint32_t epic_mask;
    trap_handler_called++;
    // Иногда есть проблемы с глобальными переменными, например epic_index
    // В начале main есть epic_index = 15, но в trap_handler он иногда равняется 8 или 6. Приходится присваивать еще раз.
    epic_index = EPIC_RTC_INDEX;  
    epic_mask = (1 << epic_index);
    xprintf("Индекс RTC(15) = %d\n", epic_index);
    ...
    EPIC->MASK_LEVEL_CLEAR = epic_mask;
    if ((EPIC->RAW_STATUS & epic_mask) != 0)
    {
...
int main()
{
    ...
    EPIC->MASK_SET = epic_mask; 
    EPIC->MASK_LEVEL_SET = epic_mask;
    RTC->CTRL |= RTC_CTRL_INTE_M;
    EnableInterrupts();
    ...
    rtc_alarm_set_time(dow, hour, minute, second + 5, alarm_time_mask);
